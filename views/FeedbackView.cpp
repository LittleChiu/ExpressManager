...
void FeedbackView::onViewPackageClicked() {
    ...
    QString info = QString("包裹ID: %1\n体积: %2\n重量: %3\n易碎: %4\n状态: %5\n位置: %6\n公司ID: %7\n收件人ID: %8\n快递员ID: %9")
        .arg(pkg.packageId).arg(pkg.volume).arg(pkg.weight).arg(pkg.isFragile ? "是" : "否")
        .arg((pkg.status == PackageStatus::STORED) ? "已存入柜" : (pkg.status == PackageStatus::PICKUPED ? "已取件" : "异常"))
        .arg(pkg.location).arg(pkg.expressCompanyId).arg(pkg.recipientId).arg(pkg.expressmanId);
