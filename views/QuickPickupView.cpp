    ...
    for (const auto &pkg: pkgs) {
        if (pkg.pickupCode == code && pkg.status == PackageStatus::STORED) {
            ...
            QString recipientStr = recipient.username.isEmpty()
                                       ? "-"
                                       : QString("%1(%2)").arg(recipient.username).arg(std::to_string(recipient.id));
            ...
            QString expressmanStr = expressman.username.isEmpty()
                                        ? "-"
                                        : QString("%1(%2)").arg(expressman.username)
            ...
            if (ret == QMessageBox::Yes) {
                ...
                bool ok2 = PackageController::instance().updatePickupTime(
                    pkg.packageId, QDateTime::currentSecsSinceEpoch());
                if (ok1 && ok2) {
                    Feedback fb;
